...
const addTrade = async (trade, user) => {
  ...
  if (!isValidUser) {
    return wrapServiceResult(null, [
      "User not allowed to trade on this portfolio",
    ]);
  }
...
const updateTrade = async (trade, user) => {
	let isValidUser = await portfolioService.isValidUser(user, trade.portfolioId);
  // portfolio doesn't belong to user
  if (!isValidUser) {
    return wrapServiceResult(null, [
	...
	return wrapServiceResult(null, ["Not implemented yet"]);
}

const getTradeByPortfolioId = async portfolioId => {
	const tradeList = await Trades.find({portfolioId});
	return wrapServiceResult(tradeList, []);
}

module.exports = {
	addTrade,
	updateTrade,
	getTradeByPortfolioId
};
